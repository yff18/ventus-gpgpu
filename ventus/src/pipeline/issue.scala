...
class Issue extends Module{
  ...
  if(SPIKE_OUTPUT) {
    ...
    when(io.out_LSU.fire&&io.out_LSU.bits.ctrl.mem/*&&io.out_LSU.bits.ctrl.wid===wid_to_check.U*/){
      printf(p"out_LSU: warp:${Decimal(io.out_LSU.bits.ctrl.wid)} ")
      printf(p"pc: 0x00000000${Hexadecimal(io.out_LSU.bits.ctrl.spike_info.get.pc)} inst: 0x${Hexadecimal(io.out_LSU.bits.ctrl.spike_info.get.inst)}")
      when(io.out_LSU.bits.ctrl.mem_cmd===1.U){
        printf(p" LSU_regid: ${io.out_LSU.bits.ctrl.reg_idxw} op${io.out_LSU.bits.ctrl.mop} base${Hexadecimal(io.out_LSU.bits.in1(0))} bias${Hexadecimal(io.out_LSU.bits.in2(0))}\n")
      }
    ...
    when(io.out_warpscheduler.fire/*&&io.out_LSU.bits.ctrl.wid===wid_to_check.U*/){
      printf(p"out_warpscheduler warp${Decimal(io.out_LSU.bits.ctrl.wid)} ")
