...
class Branch_back extends Module{
  ...
  if (SPIKE_OUTPUT) {
    when(io.out.fire/* && io.out.bits.wid === wid_to_check.U*/) {
      printf(p"branch_back: warp${io.out.bits.wid} ")
...
class Writeback(num_x:Int,num_v:Int) extends Module{
  ...
  if(SPIKE_OUTPUT){
    when(io.out_x.fire/*&&io.out_x.bits.warp_id===wid_to_check.U*/){
      printf(p"writeback_x: warp${Decimal(io.out_x.bits.warp_id)} ")
      printf(p"0x00000000${Hexadecimal(io.out_x.bits.spike_info.get.pc)} inst:0x${Hexadecimal(io.out_x.bits.spike_info.get.inst)}")
      printf(p" reg_idxw:${io.out_x.bits.reg_idxw}  wb_wxd_rd:${Hexadecimal(io.out_x.bits.wb_wxd_rd)}\n")
    }
    when(io.out_v.fire/* && io.out_v.bits.warp_id === wid_to_check.U*/) {
      printf(p"writeback_v: warp${Decimal(io.out_v.bits.warp_id)} ")
      printf(p"0x00000000${Hexadecimal(io.out_v.bits.spike_info.get.pc)} inst:0x${Hexadecimal(io.out_v.bits.spike_info.get.inst)}")
      printf(p" reg_idxw:${io.out_v.bits.reg_idxw} ")
      printf(p"wvd_mask:")
